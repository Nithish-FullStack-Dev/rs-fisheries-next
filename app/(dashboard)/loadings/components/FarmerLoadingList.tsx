...
interface Props {
  onEdit: (loading: FarmerLoading) => void;
}

export default function FarmerLoadingList({ onEdit }: Props) {
  const [search, setSearch] = useState("");
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");

  const { data: loadings = [], isLoading } = useQuery<FarmerLoading[]>({
    queryKey: ["farmer-loadings", search, fromDate, toDate],
    queryFn: async () => {
  ...
  return (
    <Card className="p-6 mt-6 rounded-2xl border border-[#139BC3]/15 bg-white shadow-sm space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <Input
          placeholder="Search Bill No / Farmer"
          value={search}
            ...
            {loadings.map((loading) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3">{loading.FarmerName}</td>
